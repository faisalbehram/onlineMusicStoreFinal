...
public class AccessoryRepoJdbc {
	...
	public List<Accessories> getAllAccessoryWithProduct() {
		String sql = "	select p.product_id,p.product_name, group_concat(a.accessory_name) as accessory ,ap.* from product as p  inner join accessory_product as ap on p.product_id = ap.product_id inner join accessories as a on a.accessory_id = ap.accessory_id group by p.product_id";
		NamedParameterJdbcTemplate namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
		...
		List<Accessories> accessories = namedParameterJdbcTemplate.query(sql,new RowMapper<Accessories>() {
			...
			public Accessories mapRow(ResultSet rs, int rowNum) throws SQLException {
					Accessories accessory = new Accessories();
					accessory.setAccessoryId(rs.getInt("accessory_id"));
					accessory.setAccessoryName(rs.getString("accessory"));
