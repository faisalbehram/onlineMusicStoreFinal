...
public class ShippingJdbcRepo  {
	...
	public ShippingAddress savingShipping(ShippingAddress shippingAddress) {
		...
		StringBuilder sql = new StringBuilder("insert into shipping_address (address, customer_id, address_type , default_address)");
		...
		parameters.addValue("default_address", 1);
	...
	public void updateShipping(int id, String shippingAddress) {
		String sql  = " update shipping_address set address = :shppingAddress where shipping_address_id = :id ";
		MapSqlParameterSource parameters = new MapSqlParameterSource();
		parameters.addValue("shppingAddress",shippingAddress);
		parameters.addValue("id", id);
		NamedParameterJdbcTemplate namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
		namedParameterJdbcTemplate.update(sql, parameters);
		System.out.println("shipping  update done ");
	}
	
	public void updateBilling(int id, String billingAddress) {
		String sql  = " update shipping_address set address = :billingAddress where shipping_address_id = :id ";
		MapSqlParameterSource parameters = new MapSqlParameterSource();
		parameters.addValue("billingAddress", billingAddress);
		parameters.addValue("id", id);
		NamedParameterJdbcTemplate namedParameterJdbcTemplate = new NamedParameterJdbcTemplate(jdbcTemplate);
		namedParameterJdbcTemplate.update(sql, parameters);
		System.out.println("billing  update done ");
	}
