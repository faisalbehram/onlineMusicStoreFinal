...
//        				user.setRoles((Set<Role>) rs.getObject("roles"));
	...
	public User save(User getuser) {
		...
		String insertSql = "INSERT  INTO user (email, password, username, verified , to_date ) VALUES (:email, :password, :username, :verified, now() )";
...
//				user.setRoles((Set<Role>) rs.getObject("roles"));
	...
	public void updateUser(Customer updateCustomer) {
		...
		StringBuilder sql = new StringBuilder(" update user set email = :userEmail where username = :username  ");
		
		MapSqlParameterSource parameters = new MapSqlParameterSource();
		
		System.out.println("the username is " + updateCustomer.getUsername() );
		parameters.addValue("userEmail", updateCustomer.getCustomerEmail());
		parameters.addValue("username", updateCustomer.getUsername());
